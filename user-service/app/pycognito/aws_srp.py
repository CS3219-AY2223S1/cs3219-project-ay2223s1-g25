...
def negative_hex(long_num, nbits):
    return long_to_hex((long_num + (1 << nbits)) % (1 << nbits))
...
def pad_hex(long_int):
    ...
    if not isinstance(long_int, six.string_types):
        ...
        long_int = int(long_int, 16)
        hash_str = negative_hex(long_int, 128)
    else:
        hash_str = long_int
    if len(hash_str) % 2 == 1:
        ...
        if response["ChallengeName"] == self.PASSWORD_VERIFIER_CHALLENGE:
            ...
            if tokens.get("ChallengeName") == self.NEW_PASSWORD_REQUIRED_CHALLENGE:
                raise ForceChangePasswordException(
                    "Change password before authenticating", tokens["Session"]
                )
